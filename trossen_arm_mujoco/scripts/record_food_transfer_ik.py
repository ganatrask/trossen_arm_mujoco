...
HDF5 structure:
    ...
    │   ├── bowl_3            (7,) float64 - [x,y,z,qw,qx,qy,qz] (if exists)
    │   └── bowl_4            (7,) float64 - [x,y,z,qw,qx,qy,qz] (if exists)
    └── attrs: sim=True, source="food_transfer_ik", target=<name>
"""
...
class FoodTransferRecorder(FoodTransferBase):
    ...
    def __init__(
        ...
        """
        super().__init__(scene_xml=scene_xml)
    ...
    def record_episode(
        ...
        is_success = (final_reward == self.max_reward)
...
def main(args):
    ...
    for episode_idx in tqdm(range(args.num_episodes), desc="Recording episodes"):
        # Determine target bowl
        if cycle_all:
            bowl_idx = episode_idx % len(ALL_BOWLS)
        ...
        if args.verbose:
            ...
            print(f"  Episode {episode_idx}: {target}, "
                  f"{num_timesteps} steps, {status}, saved in {save_time:.1f}s")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Target bowl. 'all' cycles through all 4 (default: all).",
    )
    parser.add_argument(
        "--speed",
        type=float,
